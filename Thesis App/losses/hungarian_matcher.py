...
class HungarianMatcher(nn.Module):
    ...
    def __init__(self, cost_class: float = 1, cost_bbox: float = 1):
        ...
        Params:
            ...
            cost_bbox: This is the relative weight of the L1 error of the bounding box coordinates in the matching costm
        """
        super().__init__()
    ...
    def forward(self, outputs: Dict[str, torch.tensor], targets: Dict[str, torch.tensor]):
        ...
        out_bbox = outputs["pred_points"].flatten(
            0, 1)  # [batch_size * num_queries, 4]

        # Also concat the target labels and boxes
        tgt_ids = torch.cat([v["labels"] for v in targets])
        tgt_bbox = torch.cat([v["points"] for v in targets])
        ...
        return [(torch.as_tensor(i, dtype=torch.int64, device=outputs["pred_logits"].device),
                 torch.as_tensor(j, dtype=torch.int64, device=outputs["pred_logits"].device)) for i, j in indices]
