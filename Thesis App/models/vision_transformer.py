...
# from models.layers import MemEffAttention, Mlp
# from models.layers import NestedTensorBlock as Block
# from models.layers import PatchEmbed, SwiGLUFFNFused
...
#             def f(*args, **kwargs): # pylint: disable=all
#                 return nn.Identity()
...
#         class_pos_embed = pos_embed[:, 0]   # pylint: disable=all
#         patch_pos_embed = pos_embed[:, 1:]  # pylint: disable=all
#         dim = x.shape[-1]
...
#     def forward_features_list(self, x_list: torch.Tensor, masks_list: torch.Tensor):
...
#     def forward_features(self, x: torch.Tensor, masks: torch.Tensor | None = None):
...
#     def _get_intermediate_layers_not_chunked(self, x: torch.Tensor, n:int | Sequence=1):
...
#     def _get_intermediate_layers_chunked(self, x: torch.Tensor, n: int | Sequence=1):
#         x = self.prepare_tokens_with_masks(x)
...
#             b, _, w, h = x.shape
#             outputs = [
#                 out.reshape(b, w // self.patch_size, h //
#                             self.patch_size, -1).permute(0, 3, 1, 2).contiguous()
#                 for out in outputs
#             ]
#         if return_class_token:
#             return tuple(zip(outputs, class_tokens)) # type: ignore[return-value]
#         return tuple(outputs)

#     def forward(self, *args):
#         return self.forward_features(*args)
        


# def init_weights_vit_timm(module: nn.Module, name: str = ""):   # pylint: disable=all
